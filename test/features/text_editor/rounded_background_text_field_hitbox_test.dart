...
/// Regression guard for the b3rri patch: the in-editor preview
/// ([RoundedBackgroundTextField]) must reserve the same hit-box padding around
/// its rounded background as the finished layer ([LayerWidgetTextItem], which
/// renders [RoundedBackgroundText] with `enableHitBoxCorrection: true`).
///
/// Before the patch the preview passed `enableHitBoxCorrection: false`, so the
/// background box hugged the glyphs and the reserved room differed from the
/// finished layer — the box visibly grew (and became right/left symmetric) the
